...
public class Main {
	...
	public static void main(String[] args) {
		...
		AccountService cuentaService = new AccountService();
				...
				try {
					...
					case 2:
						...
						if(clienteService.existeDNI(dni)) {
							...
							if(cliente_.getName_user().equals(usuario)&&cliente_.getPassword().equals(password)) {
								
								System.out.println("=====BIENVENIDO " + cliente_.getNombre() + " " +cliente_.getApellido() + " =======" );
								...
								if(crear_Y_N == 's') {
									...
									cuentaService.crearCuenta(cliente_, crear_Y_N);
					...
					case 3:
						...
						//registramos quien deposita
						System.out.print("Ingresa tu DNI: ");
						dni = sc.nextLine();

						System.out.print("Nro de cuenta: ");
						String nro_cuenta = sc.nextLine();

						System.out.print("Cantidad a depositar");
						double deposito = Double.parseDouble(sc.nextLine());
					
						Customer nv_cliente = clienteRepository.buscarPorDni(dni);
						
						if(!cuentaService.depositarDinero(nv_cliente, nro_cuenta, deposito)) {
